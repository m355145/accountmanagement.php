...
function check_name($name)
{
    ...
    // Permitido: letras, espaço, hífen, apóstrofo
...
// ===============================
// Funções de atualização de conta
// ===============================

// Conexão com o banco
$pdo = new PDO('mysql:host=localhost;dbname=u263410991_hardwar;charset=utf8', 'u263410991_global', '1540103jJ');

$accountId = (int)$_SESSION['user_id'];

// Processa atualização de e-mail
if (isset($_POST['update_email'])) {
    ...
    if ($newEmail) {
        ...
        echo "<p style='color: green;'>E-mail atualizado com sucesso!</p>";
    } else {
...
if (isset($_POST['update_password'])) {
    ...
    if (sha1($currentPassword) === $userPass) {
        if ($newPassword === $confirmPassword) {
            $stmt = $pdo->prepare("UPDATE accounts SET password = ? WHERE id = ?");
            $stmt->execute([sha1($newPassword), $accountId]);
            echo "<p style='color: green;'>Senha alterada com sucesso!</p>";
        } else {
            echo "<p style='color: red;'>As novas senhas não conferem.</p>";
        }
    } else {
        echo "<p style='color: red;'>Senha atual incorreta.</p>";
    }
}
...
foreach ($players_from_logged_acc as $player) {
    $name = $player->getName();
    $world = $config['site']['worlds'][$player->getWorld()];
    $status = $player->isOnline() ? '<span style="color:red;">online</span>' : '<span style="color:green;">offline</span>';
    ...
    <tr bgcolor="'.$config['site']['lightborder'].'">
        ...
        <td>'.$status.'</td>
        <td>
            <a href="index.php?subtopic=characters&name='.$name.'" class="btn">Play</a>
            <a href="index.php?subtopic=accountmanagement&action=editplayer&name='.$name.'" class="btn">Edit</a>
            <a href="index.php?subtopic=accountmanagement&action=deleteplayer&name='.$name.'" class="btn" onclick="return confirm(\'Are you sure?\');">Delete</a>
        </td>
    </tr>';
    $i++;
}
